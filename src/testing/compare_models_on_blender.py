...
VIDEO_RIGHT = r"f:\hawkeye\test-vids\finalRight.mkv"
GROUND_TRUTH_CSV = r"f:\hawkeye\3D-models\Latest volley go brr\ball_positions_blender.csv"
...
def run_evaluation():
    ...
    for model_name, model_path in MODELS.items():
        ...
        while cap_left.isOpened() and cap_right.isOpened() and frame_count < max_frames:
            ...
            # Process frame
            # We use process_from_pair which updates internal state
            # We need to extract the result. 
            # The pipeline stores results in self.ball_positions_world
            
            # Reset pipeline state for this frame if needed, but process_from_pair handles it.
            # However, pipeline accumulates results. We should clear them or just read the last one.
            
            # Actually, process_from_pair returns nothing but updates self.ball_positions_world
            # Let's look at process_from_pair implementation again.
            # It appends to self.ball_positions_world.
            
            pipeline.process_from_pair(frame_l, frame_r, frame_num=frame_count)
            
            # Get the last result
            if len(pipeline.ball_positions_world) >= frame_count:
                pos = pipeline.ball_positions_world[frame_count-1] # 0-indexed list, frame_count is 1-based
    ...
    # Print Summary
    print("\n--- Blender Footage Evaluation Summary ---")
    summary_df = pd.DataFrame(results)
    print(summary_df.to_string(index=False))
    
    # Save to CSV
    summary_df.to_csv(r"f:\hawkeye\output\blender_model_comparison.csv", index=False)
    print(f"\nSaved results to f:\hawkeye\output\blender_model_comparison.csv")
